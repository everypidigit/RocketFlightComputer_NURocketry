...
/* Edit Date: August 4th, 17:15

  Important Edits: added the SD card function implementation, added the functionality to the take_mpu_data().
  Bugs?: dunno, never ran the file. no place to run on. but I more than assume that there is a shit ton of bugs
...
//completed but has to be tested
...
//up to be implemented by Alisher
...
//ready, I guess? gotta do a lot of testing though, huh. and I have to come up with a clever loop condition.
void transfer_to_card(double data[]) {

  logFile = SD.open("test.txt", FILE_WRITE);
  
  // here has to be a while/for loop that will do the data writing until the referenced array is empty
  // some time later
  logFile.print(data[0]);
  logFile.print(" / ");
  logFile.print(data[1]);
  logFile.print(" / ");
  logFile.println(data[2]);
  logFile.print(" / ");
