...
double VelocityY = AccY*(elapsedTime); // might be used in the parachute deployment control loop
...
void loop() {
  ...
  */
  ...
  /* The following comment is about the parachute deployment condition.
    ...
    After the rocket engine's work will end and it will stop providing thrust, the acceleration becomes -g instantly and the velocity that the rocket has accumulated starts decaying until it reaches 0.
    At 0 velocity we register the flight apogee, after which the rocket's velocity should start become more negative. At that moment, when the flight computer registers first negative velocity reagings, the rocket will fire the drogue event.
...
void get_pressure(int time){ 
  ...
  sensors_event_t event; //creating a sensor event of type barometer
  bmp.getEvent(&event); //using the event
